...
/// Arranger strategies. Mirrors `src/llm/configs.ts` on the web side.
///
...
enum PlannerConfig: String, CaseIterable, Identifiable {
    case vibe1
    case realvibe
    case sounds
    case continuity
    ...
    static let `default`: PlannerConfig = .vibe1
    ...
    var strategy: String {
        switch self {
        case .vibe1: return Self.vibe1Strategy
        case .realvibe: return Self.realvibeStrategy
        case .sounds: return Self.soundsStrategy
        case .continuity: return Self.continuityStrategy
        }
    }

    private static let vibe1Strategy = """
        STRATEGY — realign the mix to the whole drawing
    ...
    private static let realvibeStrategy = """
        STRATEGY — mirror the drawing, always
    ...
    private static let soundsStrategy = """
        STRATEGY — sound, not music
    ...
    private static let continuityStrategy = """
        STRATEGY — continue or replace
